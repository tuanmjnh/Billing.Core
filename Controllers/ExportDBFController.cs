...
namespace Billing.Core.Controllers {
    ...
    public class ExportDBFController : BaseController {
        ...
        public JsonResult GetTableList(string database) {
            ...
            try {
                ...
                var data = _Con.Connection.Query<Models.TABLES>(qry).ToList();
        ...
        public JsonResult GetDetailsTableExport(string tableName) {
            ...
            try {
                ...
                var data = _Con.Connection.Query<Models.COLUMNS>(qry).ToList();
                //
                qry = $"SELECT * FROM EXPORT_TABLE WHERE TABLE_TYPE={(int)Common.Objects.TABLE_TYPE.DBF} AND TABLE_NAME='{tableName}'";
                var listETOld = _Con.Connection.Query<Models.EXPORT_TABLE>(qry).ToList();
                //
                ...
                _Con.Connection.Insert(listET);
                listET = _Con.Connection.Query<Models.EXPORT_TABLE>(qry).ToList();
                return Json(new { data = listET, success = "Xử lý thành công!" });
            } catch (Exception ex) { return Json(new { danger = ex.Message + " - Index: " + index }); }
        }
        ...
        public JsonResult ExportToDBF(Common.DefaultObj obj) {
            ...
            try {
                ...
                var EXPORT_TABLE = _Con.Connection.Query<Models.EXPORT_TABLE>(qry).ToList();
                //
                if (EXPORT_TABLE.Count <= 0)
                    return Json(new { warning = "Vui lòng cấu hình dữ liệu Export trước!" });
                ...
                var data = _Con.Connection.Query<dynamic>(qry).ToList();
                //FoxPro.Insert(data);
                //
        ...
        public JsonResult CreateExport(Common.DefaultObj obj) {
            ...
            try {
                ...
                var data = _Con.Connection.Query<Models.EXPORT_TABLE>(qry).ToList();
                foreach (var i in data) {
        ...
        public JsonResult CreateExportSave(ExportCustom obj, List<Models.EXPORT_TABLE> DataList) {
            ...
            try {
                if (string.IsNullOrEmpty(obj.ExportName))
                    return Json(new { danger = "Vui lòng nhập tên kết xuất!" });
                ...
                var EXPORT_CUSTOM = _Con.Connection.QueryFirstOrDefault<Models.EXPORT_CUSTOM>(qry);
        ...
        public JsonResult GetExportCustom() {
            try {
                ...
                var data = _Con.Connection.Query<Models.EXPORT_CUSTOM>(qry);
                return Json(new { data = data, roles = Authentication.Core.Auth.AuthUser.roles, success = "Cập nhật thành công!" });
            } catch (Exception ex) { return Json(new { danger = ex.Message }); }
        }
        ...
        public JsonResult EditExportCustom(Guid id) {
            ...
            try {
                ...
                return Json(new { EXPORT_CUSTOM = EXPORT_CUSTOM, EXPORT_TABLE = EXPORT_TABLE, success = "Cập nhật thành công!" });
            } catch (Exception ex) { return Json(new { danger = ex.Message }); }
        }
        ...
        public JsonResult ExportExportCustom(Common.DefaultObj obj) {
            ...
            try {
                ...
                //Remove Old File
                TM.Core.IO.Delete($"{obj.DataSource}{EXPORT_CUSTOM.TABLE_NAME}.dbf", false);
                //Create New File
                //FoxPro.Connection.Query(EXPORT_CUSTOM.QUERY_CREATE);

                var data = _Con.Connection.Query(EXPORT_CUSTOM.QUERY_SELECT.Replace("$timebill", obj.month_year_time)).ToList();
                qry = $"SELECT * FROM EXPORT_TABLE WHERE APP_KEY='{EXPORT_CUSTOM.ID}' AND TABLE_TYPE={(int)Common.Objects.TABLE_TYPE.EXPORT_CUSTOM}";
                var EXPORT_TABLE = _Con.Connection.Query<Models.EXPORT_TABLE>(qry).ToList();
                ...
                return Json(new { success = "Cập nhật thành công!", url = UrlDownloadFiles($"{Common.Directories.HDData}{obj.time}\\{EXPORT_CUSTOM.TABLE_NAME}.dbf", $"{EXPORT_CUSTOM.TABLE_NAME}{obj.datetime.ToString("yyyyMM")}.dbf") });
            } catch (Exception ex) { return Json(new { danger = ex.Message }); }
        }
        ...
        public JsonResult RemoveExportCustom(Common.DefaultObj obj) {
            ...
            try {
                ...
                _Con.Connection.Query(qry);
                return Json(new { success = "Xóa kết xuất thành công!" });
            } catch (Exception ex) { return Json(new { danger = ex.Message }); }
        }
        ...
        private string CreateTable(string DataSource, string tableName, List<Models.EXPORT_TABLE> tableCol) {
            try {
                TM.Core.IO.Delete(DataSource + tableName, false);
        ...
        //        FileManagerController.InsertFile($"{obj.DataSource}{obj.file}.dbf", false);
        //        return Json(new { success = "Xử lý thành công!", url = UrlDownloadFiles($"{Common.Directories.HDData}{obj.time}\\{obj.file}.dbf", $"{obj.file}.dbf") });
        //    }
        //    catch (Exception ex) { return Json(new { danger = ex.Message + " - Index: " + index }); }
        //    finally { _Con.Connection.Close(); FoxPro.Close(); }
        //}
        //[HttpPost, ValidateAntiForgeryToken]
    ...
    public static class CRUDFoxPro {
        ...
        public static string TypeNameLower(object obj) {
            if (obj != null) { return obj.GetType().Name.ToLower(); } else { return null; }
        }
