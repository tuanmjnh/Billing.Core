    ...
    public class TSLController : BaseController {
        ...
        public JsonResult UpdateContactData(Common.DefaultObj obj) {
            ...
            try {
                ...
                var dbpttb = HNIVNPTBACKAN1.Connection.Query<Models.DANH_BA_TSL>(qry);
                //Get data DB_DUONGTHU_BKN
                qry = $"SELECT * FROM DB_DUONGTHU_BKN";
                var dbdt = _Con.Connection.Query<Models.DB_DUONGTHU_BKN>(qry);
                //Get data DB KH
                ...
                var tslRemove = _Con.Connection.Query<Models.HD_TSL>(qry);
                //
                ...
                //
                if (DataInsertHD.Count > 0) _Con.Connection.Insert(DataInsertHD);
                //UPDATE NULL
                ...
                _Con.Connection.Query(qry);
                //
                return Json(new { success = $"TSL - Cập nhật: {DataUpdate.Count} - Thêm mới: {DataInsert.Count}" });
        ...
        public JsonResult UpdatePrice(Common.DefaultObj obj) {
            ...
            try {
                ...
                _Con.Connection.Query(qry);
                return Json(new { success = $"HD_TSL - Cập nhật giá thành công!" });
            } catch (Exception ex) { return Json(new { danger = ex.Message + " - Index: " + index }); }
        }
