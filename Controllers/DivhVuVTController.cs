    ...
    public class DivhVuVTController : BaseController {
        public ActionResult Index(int? flag, string order, string currentFilter, string searchString, int? page, string datetime, int? datetimeType, string export) {
            try {
                ...
                var rs = _Con.Connection.Query<Models.DICHVU_VT_BKN>("SELECT * FROM DICHVU_VT_BKN WHERE FLAG>0"); //.DICHVU_VT_BKN.Where(m => m.FLAG > 0);
                ...
                if (!String.IsNullOrEmpty(datetime)) {
                    ...
                    if (date.Length > 1) {
                        var dateStart = TM.Core.Format.Formating.StartOfDate(TM.Core.Format.Formating.DateParseExactVNToEN(date[0]));
                        var dateEnd = TM.Core.Format.Formating.EndOfDate(TM.Core.Format.Formating.DateParseExactVNToEN(date[1]));
                ...
                if (!String.IsNullOrEmpty(export)) {
                    //TM.Core.Exports.ExportExcel(TM.Helper.Data.ToDataTable(rs.ToList()), "Dịch vụ viễn thông");
        ...
        public ActionResult Update() {
            TM.Core.Connection.Oracle HNIVNPTBACKAN1 = new TM.Core.Connection.Oracle("HNIVNPTBACKAN1");
            try {
                var qry = "SELECT a.*,a.VI_TRI AS ORDERS FROM DICHVU_VT_BKN a";
                var hni = HNIVNPTBACKAN1.Connection.Query<Models.DICHVU_VT_BKN>(qry).ToList();
                qry = "SELECT * FROM DICHVU_VT_BKN";
                var data = _Con.Connection.Query<Models.DICHVU_VT_BKN>(qry).ToList();
                //
                if (hni.Count < 0) {
        ...
        public async Task<JsonResult> update_flag(string uid) {
            try {
                var data = _Con.Connection.Query<Models.DICHVU_VT_BKN>($"SELECT * FROM DICHVU_VT_BKN WHERE IN({uid})").ToList();
                var FLAG = 0;
                foreach (var i in data)
                    i.FLAG = FLAG = i.FLAG == 1 ? 0 : 1;
                await _Con.Connection.UpdateAsync(data);
                return Json(new { success = (FLAG == 0 ? "Xóa thành công!" : "Khôi phục thành công") });
            } catch (Exception) { return Json(new { danger = "Lỗi trong quá trình thực hiện" }); }
        }
    }
}